...
class StockInfoScraper(val exchangeName: String, val symbols: Option[Array[String]]) extends ParserUtils with ParserImplicits {
  ...
  def run: Boolean = {
    ...
    stocks.foreach { stockSymbol =>
      ...
      Jsoup.connect(GoogleFinanceUrl).timeout(ConnectionTimeout).ignoreContentType(true)
        .userAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:40.0) Gecko/20100101 Firefox/40.1").get()

      try {
        log.info(s"Grabbing stock info for $exchangeName - $symbol")
        val document: Document = Jsoup.connect(GoogleFinanceUrl).timeout(ConnectionTimeout).ignoreContentType(true)
          .userAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:40.0) Gecko/20100101 Firefox/40.1").get()
